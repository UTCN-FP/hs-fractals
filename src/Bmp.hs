...
data Pixel = Pixel {r :: W.Word8, g :: W.Word8, b :: W.Word8}
...
getDibHeaderBytes :: DibHeader -> BSB.Builder
...
createFileHeader :: DibHeader -> FileHeader
...
createDibHeader :: BmpImg -> DibHeader
createDibHeader (BmpImg img) =
  DibHeader
    { headerSize = dibHeaderSize,
      ...
      nrColorPlanes = 1,
      bitsPerPixel = 3 * 8
    } where (ImgRow firstRow) = head img

getImgDataBytes :: BmpImg -> BSB.Builder
getImgDataBytes (BmpImg img) =
  let
    getRowBytes :: ImgRow -> BSB.Builder
    getRowBytes (ImgRow row) =
      padRow (mconcat [mconcat $ map BSB.word8 [b px, g px, r px] | px <- row]) rowLen where
        rowLen = length row

    padRow :: BSB.Builder -> Int -> BSB.Builder
    padRow row rowLen = 
      let
        paddedSize = roundUp rowLen 4
      in
        if paddedSize > rowLen then
          row `mappend` mconcat (replicate (fromIntegral (paddedSize - rowLen)) (BSB.int8 0))
        else
          row
  in
    mconcat $ map getRowBytes img
